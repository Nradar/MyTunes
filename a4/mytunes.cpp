...
void MyTunes::executeCMDFollow(Command cmd){
    //follow -u jjj -p fav -f kkk
    enum arguments {FOLLOW, _U, USER_ID, _P, PLAYLIST_NAME, _F, FOLLOW_ID};
    
    //cout << "execute cmd following" << endl;
    
    if(cmd.getToken("-f").compare("stop")==0){
        //stop following
        User * userA = users.findByUserID(cmd.getToken("-u"));
        if(userA == NULL){
            view.printOutput("User ID doesn't exist");
            return;
        }
        Playlist * playlistOfUserA = userA->findPlaylist(cmd.getToken("-p"));
        if(playlistOfUserA == NULL){
            view.printOutput("Error Input, playlist doesn't exist");
            return;
        }
        playlistOfUserA->stopFollowing();
        view.printOutput("EXECUTING: STOP FOLLOWING " + cmd.getCommandString());
        //playlistOfUserA->getFollowingList()->dettach(*playlistOfUserA);
    }else{
        // check the format
        if(!cmd.isTokenAt(_U, "-u") || !cmd.isTokenAt(_P, "-p") || !cmd.isTokenAt(_F, "-f")){
            view.printOutput("Error Command Input");
            return;
        }
        // check if input users are exist
        User * userA = users.findByUserID(cmd.getToken("-u"));
        User * userB = users.findByUserID(cmd.getToken("-f"));
        if(userA == NULL || userB == NULL){
            view.printOutput("Error ID Input");
            return;
        }
        // check if two users are the same one
        if(userA == userB){
            view.printOutput("Error ID input, Same User ID");
            return;
        }
        // check if playlist is exist in both user
        Playlist * playlistOfUserA = userA->findPlaylist(cmd.getToken("-p"));
        Playlist * playlistOfUserB = userB->findPlaylist(cmd.getToken("-p"));
        if(playlistOfUserA == NULL && playlistOfUserB == NULL){
            view.printOutput("playlist doesn't exist in both users");
            return;
        }
        if(playlistOfUserA==NULL){
            cout << "playlist doesn't exist in " << userA->getUserID() << endl;
            return;
        }
        if(playlistOfUserB==NULL){
            cout << "playlist doesn't exist in " << userB->getUserID() << endl;
            return;
        }
        if(playlistOfUserA->checkExist(*playlistOfUserB)){
            view.printOutput("Mutual Link Error");
            return;
        }
        //cout << *playlistOfUserA << endl;
        //cout << *playlistOfUserB << endl;
        playlistOfUserA->startFollowing(*playlistOfUserB);
        view.printOutput("EXECUTING: START FOLLOWING " + cmd.getCommandString());
    }
}
