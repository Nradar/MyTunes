...
void Playlist::update(Subject * subject){
    Playlist * pl = (Playlist *) subject;
    cout << this << " Playlist::update(Subject * subject)" << endl;
    tracks.clear();
    for (vector<Track*>::size_type i = 0 ; i < pl->getTracks().size(); i++){
        tracks.push_back(pl->getTracks()[i]);
    }
}
...
void Playlist::startFollowing(Playlist& followPlaylist){
    cout << this << " Playlist::startFollowing(Playlist & aPlaylist)" << endl;
    //this->tracks = followPlaylist.getTracks();
    followPlaylist.attach(*this);
    update(&followPlaylist);
    followingList = &followPlaylist;
}
